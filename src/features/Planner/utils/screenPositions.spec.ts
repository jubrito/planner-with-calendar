...
describe('screenPositions', () => {
  ...
  describe('calculateYPosition', () => {
    it('should create event that start on first hour and end on midnight of current day', () => {
      ...
      expect(yPosition).toStrictEqual({
        startY: 0,
        endY: plannerContainerSize,
      });
    });
    it('should create event that start on last hour and end on midnight of current day', () => {
      ...
      const startDate = getDateISOString(
        new Date(year, month, day, startHour, minute),
      );
      const endDate = getDateISOString(
        new Date(year, month, day, endHour, minute),
      );
      const yPosition = calculateYPosition(startDate, endDate);
      expect(yPosition).toStrictEqual({
        startY: sizeOfEachHourBlock * startHour,
        endY: plannerContainerSize,
      });
    });
    it('should create events with a minimum size of 15 minutes', () => {
      ...
      const minutes = 0;
      const startDate = getDateISOString(
        new Date(year, month, day, startHour, minutes),
      );
      const endDate = getDateISOString(
        new Date(year, month, day, endHour, minutes),
      );
      const yPosition = calculateYPosition(startDate, endDate);
      const startY = sizeOfEachHourBlock * startHour;
      expect(yPosition).toStrictEqual({
        startY: startY,
        endY: startY + sizeOfEach15MinBlock,
      });
    });
  });
});
